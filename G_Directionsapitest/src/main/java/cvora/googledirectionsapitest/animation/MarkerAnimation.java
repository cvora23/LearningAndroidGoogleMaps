package cvora.googledirectionsapitest.animation;
...
public class MarkerAnimation {
    ...
    public static void animateGivenRoutingPoints(final Marker marker, final List<LatLng>routingPts){

        shutdown = false;
        final Handler handler = new Handler();
        final long start = SystemClock.uptimeMillis();
        final int totalPts = routingPts.size();
        ...
        handler.post(new Runnable() {
            ...
            public void run() {
                if(!shutdown){
                    if(count < totalPts){
                        marker.setPosition(routingPts.get(count));
                        WayPointMgr.getInstance().setCrossed(routingPts.get(count));
                        count++;
                        // Post again ANIMATION_DELAY later. Typically 16 ms to get 60 frames per second.
